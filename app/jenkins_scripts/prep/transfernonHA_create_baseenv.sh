...
echo "This is the ABCD remote host: "
echo "Nodes: "$nodes
...
#        echo "Copying files to automate VMs deployment on ABCD host"
	...
	echo "after the copy change local folder in perf_config_reader.sh "
        sshpass -p $PASS ssh root@$host_name  sed -i.bak 's/app/ecm-umi/g' /ecm-umi/install/kvm/Installation/replace_baseenv.nonHA.sh
	sshpass -p $PASS ssh root@$host_name  sed -i.bak 's/jenkins_scripts/install/g' /ecm-umi/install/kvm/Installation/replace_baseenv.nonHA.sh
	sshpass -p $PASS ssh root@$host_name  sed -i.bak 's/prep/kvm/g' /ecm-umi/install/kvm/Installation/replace_baseenv.nonHA.sh
	...
	echo " Run script replace_baseenv.nonHA.sh on ABCD host"
#        sshpass -p $PASS ssh -oStrictHostKeyChecking=no root@$host_name uptime
	eval sshpass -p $PASS ssh root@$host_name /ecm-umi/install/kvm/Installation/replace_baseenv.nonHA.sh -nodes_yaml $2 -config_file $3 -release $4 -drop $5 -system $6 -stage_dir $7 -stage_blade $8
#	eval sshpass -p $PASS ssh root@$host_name ls -rlt /ecm-umi/install/kvm/baseenv.nonHA
        echo "Done."
done
